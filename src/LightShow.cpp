...
void LightShow::glowing(struct PIXEL_COLOR *c, uint16_t d, uint8_t* level, ADJUST_FUNC func) {
    ...
    for(int i : range(3)) setBrightness(d, i % 2, level, func);
    *level = 0; //set default brightness level
}

void LightShow::setBrightness(uint16_t d, bool up, uint8_t* level, ADJUST_FUNC func) {
    br_iterator iter(up);
    ...
    *level = iter.begin();
    for(int i=0; i < 255 ; i++) {
        *level = iter.next();
        ...
        pixels->setBrightness(func(*level));
        pixels->show();
        delay(d);
    }
    *level = iter.end();
